...
// picker + quantity + price + remove. The image/qty/price/remove columns alone
...
// scroll on mobile, so below 640px the row restacks onto two lines.
//
...
// five children carry a class so the mobile grid can place them by area. The
// classes are inert wherever there's no `.scl` grid parent (e.g. the table
// layout reuses the thumbnail renderer), so they're safe to apply everywhere.
...
@media (max-width:640px){
  .scl{
    ...
    grid-template-areas:"thumb main price" "thumb qty remove";
