...
import { resolveCardTemplate, buildCardContext, buildTagMaps, renderCards, MinimalCard } from '@/modules/shop/lib/card-template'
...
export async function ShopProductCardRsc(props: ShopProductCardProps) {
  ...
  const item = { product, ctx: buildCardContext(product, media, tagById, tagIds, config.currencySymbol, pricing, fromPrices.get(product.id) ?? null, cardExtras.get(product.id), tagsById) }
