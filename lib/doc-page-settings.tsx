...
// and the proforma - and the reader that turns them into print instructions.
//
// These are settings that belong to the SHEET rather than to any block on it:
// what size paper it is, which way up, and how much white space runs round the
// outside. Nothing on the document can express those, because none of them is
// something a block draws - the margins of a PDF are decided by the browser
// printing it, not by the markup.
//
// So they live on the layout's ROOT, which core now lets a module declare
// (`pageSettings` in cactus.module.json → lib/puck/module-layout-roots.ts). In
// the layout editor they are the fields shown with nothing selected; on the
// published document they become the @page rule below, so pressing Ctrl+P in a
// browser gives the same margins the Download PDF button does; and
// `docPageSetup` reads them back out of the saved layout for the headless
// browser that makes the file.
//
// CLIENT-SAFE, and it has to stay that way: this file is reached from the Puck
// editor bundle as well as from the server. No next/headers, no prisma, no
// module of ours that touches either.
...
export function PdfFooterRegion({ children }: { children: ReactNode }) {
 ...
 *  is the figure `renderInvoicePdf` used to hard-code, so a layout nobody has
 ...
 *
 * Rendered ahead of the document's own blocks by core's module-layout root. It
 * is a document-wide at-rule and cannot be scoped to a subtree, which is exactly
 * why it is here on the root and not on a block: one document, one sheet.
 */
export function ShopDocPageStyle(props: DocPageProps) {
...
/** The root config core hands to Puck for the invoice, credit note and proforma
 *  layout types. */
...
export const shopDocFooterPageSettings = {
  fields: {
...
export function ShopDocFooterPageStyle(props: { align?: string; inset?: string }) {
