...
| `shop.order-line-due-date` | Answers, per order line, the day that line was promised to arrive, so the orders list's "Delivery due" day covers items not dispatched yet as well as parcels with a booked day. Server-only (`serverOnly: true`). For a module whose promise lives in its own `line_meta.data`, which shop never reads. See `lib/order-line-due-date.ts`. |
| `shop.product-storefront-link` | Answers, per product id, which storefront page opens it and with what query string - so the admin order screen opens a variation on its parent listing with that combination chosen, rather than the child's editor. Server-only (`serverOnly: true`). Shop applies the URL style; unanswered ids fall back to the product's own slug. See `lib/product-storefront-link.ts`. |
...
| `shop.settings-notifications` | Hosts another module's settings panel at the foot of Shop's **Notifications** settings sub-tab, for anything that has something to say about the shop's outgoing email. Merged shape - the panel writes its own heading. |
